...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    async function loadAnalytics() {
      ...
      try {
        const [summaryRes, timelineRes, rankingRes, activityRes] = await Promise.all([
        ...
        const failed = [summaryRes, timelineRes, rankingRes, activityRes].find((result) => result.error);
        ...
        setRankings([]);
        setActivity([]);
        setError(loadError?.message?.includes('function')
          ? '고급 통계용 데이터베이스 기능이 아직 적용되지 않았습니다. database/add_admin_analytics.sql을 실행해 주세요.'
          : (loadError?.message || '고급 통계를 불러오지 못했습니다.'));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <section style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ opacity: loading ? 0.45 : 1, transition: 'opacity 0.2s', display: 'flex', flexDirection: 'column', gap: '16px' }} aria-busy={loading}>
        ...
        <div style={CARD_STYLE}>
          <h4 style={{ margin: '0 0 14px', color: 'var(--admin-text-main)' }}>행동 추이</h4>
        ...
        <div style={CARD_STYLE}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: '14px' }}>
            <div><h4 style={{ margin: 0, color: 'var(--admin-text-main)' }}>통합 활동 로그</h4><span style={{ color: 'var(--admin-text-sub)', fontSize: '0.8rem' }}>최대 150건 · 최신순</span></div>
            <button type="button" className="analytics-preset-button" disabled={!activity.length} onClick={exportActivity}>CSV 내보내기</button>
          </div>
...
function RankingTable({ title, rows, empty }) {
  return <div style={CARD_STYLE}><h4 style={{ margin: '0 0 14px', color: 'var(--admin-text-main)' }}>{title}</h4>

...
2. ⚡ **'찜하기'** 버튼 클릭 (30분간 유효)
3. 🏃 오피스 아워 내에 **동아리방 방문** 후 키오스크로 '대여 확정'

현재 무료 대여 기간 및 오피스 아워는 **1주차 월요일~금요일** 오후 6시 ~ 오후 9시입니다. 
무료 대여를 체험하는 분들은 오피스 아워 내 반납하거나, 자신의 이름을 표시해 동아리 앞 의자에 올려두시면 반납 처리를 도와드리겠습니다.

**덜지니어스 부원이 되면 지금부터 학기 내내, 오피스아워 제한 없이 무제한 대여 가능!**
...
1. 용어 정리
  1.1 '덜지니어스 내부의 대여'는 다음을 의미한다.
    ...
    2. 임원단과의 접촉으로부터 이루어진 단기 대여 (동아리, 외부 단체 등)
...
2. 대여
  ...
  보드게임을 대여할 때는 대여 플랫폼 [덜지니어스 대여소]를 활용함을 원칙으로 한다. 
    ...
    2. 소유자가 있는 보드게임의 경우, 해당 인물에게 연락을 취한 뒤 동의를 받은 후 대여함을 원칙으로 한다. 
      2-1 소유자의 허가 여부에 따라, 소유자와의 연락이 없이 보드게임을 자유롭게 빌려주도록 허가할 수 있다. (2026.04.11 수정)
      2-2 소유자는 대여 기능을 정지하는 것으로 동아리방에 보드게임 외부로 보드게임을 빌려주는 것을 불허할 권리가 있다. (2026.04.11 수정)
    3. 보드게임 소유자가 개인의 보드게임에 대해 반납을 요청할 시 대여자는 가능한 빠르게 보드게임을 반납해야 한다.
    
  2.1. 대여의 불허 
    다음과 같은 상황에서는 대여의 순서를 조정하거나 일시적으로 대여를 불허한다.
    1. 개인 소유 보드게임을 소유자 본인이 사용하는 경우
    2. 사랑의 마라톤, 새섬위원회, 축제 기간 등 교내 행사 및 공익적 목적 사용의 경우. (이하 단기 대여)
      2-1 단기 대여 승인의 기준은, 임원단의 내부 회의를 그 근거로 하며, 그 사실을 이해관계 당사자에게 알려야 한다. 
  ...
  2.2 대여의 기간.
    ...
    2. 단기 대여
      단기 대여의 경우 기간의 제한 없이 보드게임을 대여해줄 수 있다. 더불어, 필요하다 판단될 시 기간의 연장을 신청할 수 있다.
      그러나, 단기 대여가 부원들이 보드게임을 대여할 수 있는 권리를 침해할 수 있고, 보드게임 파손의 염려가 있기에, 임원단은 다음과 같은 정보를 요청할 수 있다.
      1. 행사의 개요
      2. 일정 계획
      3. 예상 보드게임 플레이어 
      단기 대여의 경우, 대여와 반납의 기간을 명확하게 공지하여, 부원의 불만을 최소화하여야 한다.
  ...
  다음의 경우 그 기간을 연장할 수 있다.
    ...
    두 연장의 요청 모두 2.1 대여의 불허 조건을 만족할 경우 거부될 수 있으며, 연장 거부 사실을 몰랐더라도, 반납이 늦어진 기간에 비례해 벌금이 주어질 수 있다.
...
4. 배상 
  4.1 현물 배상의 원칙 (우선 적용)
    ...
    절판 등의 사유로 신제품 구매가 불가능할 경우, 운영진의 검수 하에 상태가 양호한 중고 제품으로 대체할 수 있다.
  ...
  4.2 현금 배상 
    ...
    1. 구매 불가능 보드게임
    보드게임이 절판되어 시중에서 구할 수 없는 경우, 최초 구매가(원가)의 200%~400% 범위 내에서 운영진이 책정한 금액을 배상한다. (프리미엄 고려)
    2. 부분 파손에 대한 보상금
    게임 진행에는 지장이 없으나 미관상 손해를 입힌 경우(박스 백화, 찢어짐 등). 이 경우 운영진의 심의를 통해 **현재 온라인 최저가의 100% 이하의 금액**을 동아리에 납부한다.
    3. 구매 가능하나 현금 배상을 희망하는 경우
    부득이한 사정으로 대여자가 직접 구매가 어려워 운영진에게 구매를 위탁하는 경우, 현재 온라인 최저가 + 배송비 + 5,000원을 배상해야 한다.
    4. 어떠한 경우에도 현금 배상은 보드게임 원가의 400%를 넘을 수 없다.
    5. 운영진의 심의 결과는 절대적인 기준으로, 대여자는 이를 납득하여야 한다.
       한편, 운영진 또한 보드게임이 결국 소모품이라는 사실을 인정하고, 보드게임 대여자에게 있어 과한 부담이 되지 않도록 숙고하여야 한다.

  4.3 배상의 항소 및 처리    
    동아리 소유 보드게임의 배상 판정에 불만이 있을 경우 운영진에게 재심의를 요청할 수 있다.
    정당한 파손/분실 보고 절차를 통해 보고되었고, 심각한 수준의 파손이 아니라면, 
    운영진은 4.2의 5항을 신중히 고려하여 부원의 부담을 줄일 책임이 있고, 부원은 결과에 납득해야 한다.

5. 벌금
  부원과 외부인 모두 보드게임 연체 및 시설 파손 등 기타 사유에 대하여 벌금을 부과할 수 있다.
    1. 단기 대여의 연체
      1.1 연체 시 벌금의 계산 
        연체시 대여비의 50%가 누산된다. 이 금액은 반납 시 낼 의무가 있다. 금액 누산은 3일마다 반복된다. 
        예를 들어, 3/1일에 빌린 게임은 3/2일 24시 이전에 반납하면 누산이 없고, 3/3일 0시부터 추가 금액이 붙는다. 3일이 지난 3/6일에도 마찬가지로 금액이 누산된다.
      1.2 연체의 종료 조건 
        연체는 보드게임의 반납은 물론, 벌금의 납부가 완료되었을 때 종료된 것으로 본다.
        대여자가 벌금을 내지 않은 경우는, 보드게임을 반납하지 않은 것으로 본다.
        해당 경우 소속 단체 등으로 공식적인 연락을 취할 수 있다.
      1.3 '도난' 상황으로의 연결
        대여자가 2차 누산 시점까지 연락이 두절된 경우, '도난' 상황으로 간주하고 6. 도난에 준하여 처리한다.
    2. 부원의 연체
    부원의 경우에는 내부 포인트 제도를 통해 반납 연체에 대한 패널티를 진다. 
    한 학기 동안 3번의 연체 시, 다음 학기까지 보드게임 대여가 불가능하다.
    3. 기타 시설 파손 등
    사적인 물품의 시설 파손 변상은 당사자간의 이해관계를 우선시하여 고려하나,
    동아리 물품의 파손은 보드게임과 마찬가지로 4. 배상에 준하여 처리할 수 있다.
    4. 벌금의 처리
    벌금은 덜지니어스에서 보드게임을 구매하는 데 사용해야 한다.
    5. 연체의 항소 및 처리
    연체 및 벌금에 대한 판단에 불만이 있을 경우 운영진에게 재심의를 요청할 수 있다. 
    연체의 사유가 정당하다고 판단될 경우, 운영진은 벌금을 면제하거나 줄일 수 있다.
    

6. 도난
    1. 도난의 정의
    반납 기한이 지났음에도 불구하고, 운영진의 연락을 받지 않거나 연락이 두절되었다고 판단된 시점으로부터 일주일(7일)이 경과한 경우를 도난 상황으로 간주한다.
    2. 유관 부서 협조 및 신상 정보 활용
    운영진은 보드게임 회수 및 대여자 확인을 위해 교내 유관 부서(학생처 등), 소속 학부 및 담당 교수님께 협조를 요청할 수 있으며, 이 과정에서 불가피하게 대여자의 신상 정보가 활용될 수 있다.
    모든 사건이 해결된 이후, 운영진은 수집/활용된 신상 정보를 즉시 폐기해야 한다.
    3. 법적 조치
    운영진은 도난 상황 발생 시 경찰 신고 등 민/형사상 법적 조치를 취할 수 있다.

    `
};

// [NEW] 외부 단기 대여 페이지 안내문구 모음
export const ORG_RENTAL_TEXTS = {
  TITLE: "🏢 단기 대여 안내",
  GUIDE_TITLE: "📌 대여 안내 및 규정",
  GUIDE_DESC: "한동대학교 총학생회, NGO 등 학내외 자치 기구 및 동아리, 교회 단체 행사 등을 위한 보드게임 단기 대여를 지원합니다. 원활한 대여 진행을 위해 아래 규정을 숙지해 주시기 바랍니다.",
  RULES: [
    { label: "신청 기간", desc: "26-1학기 내내, 픽업 희망일 최소 3일 전까지 신청" },
    { label: "대여 기간", desc: "1박 2일 (다음날 밤 12시까지), 협의 후 연장 가능" },
    { label: "대여비", desc: "게임 2개 4,000원, 게임 4개 7,000원, 게임 6개 10,000원" },
    { label: "수령/반납 장소", desc: "학관 2층 231호 덜지니어스 동아리방" }
  ],
  CAUTION_TITLE: "🚨 주의사항",
  CAUTIONS: [
    "컴포넌트(말, 카드 등) 분실 및 파손 시 규정에 따라 배상해야 합니다.",
    "반납 기한 연체 시 벌금이 부과됩니다."
  ],
  BTN_APPLY: "📝 단기 대여 신청서 작성하기"
};
